...
export class BookDetailView extends NixComponent {
    ...
    private checkingStatus = signal(isLoggedIn.value);
    private reserved = signal(false);
    private postReserveStock = signal<number | null>(null);
    private userHasActiveInfo = signal<Reservation | null>(null);
    ...
    private checkActiveStatus = async (bookId: number) => {
        ...
        if (!uid) { this.checkingStatus.value = false; return; }
    ...
    private handleReserve = async (book: any) => {
        ...
        try {
            ...
            const current = this.postReserveStock.value !== null ? this.postReserveStock.value : book.stock;
            this.postReserveStock.value = current - 1;
            ...
            showToast(err instanceof Error ? err.message : "Error al reservar", "error");
        } finally {
    ...
    onInit() {
        if (isLoggedIn.value) this.checkActiveStatus(this.bookId);
    }

    private renderActionButtons(book: any): NixTemplate {
        ...
        if (!isLoggedIn.value) {
            return html`
                ...
                >🔐 Iniciar sesión para reservar</button>
            `;
        }

        const liveStock = () => this.postReserveStock.value !== null ? this.postReserveStock.value : book.stock;

        return html`
            <div class="flex flex-col gap-3 items-start">
                <button
                    class=${() => `px-8 py-4 text-sm font-black uppercase tracking-widest rounded-2xl transition-all focus:ring-4 focus:ring-indigo-500/20 ${
                        liveStock() > 0 && !this.loading.value
    ...
    private renderBook(book: any): NixTemplate {
                ...
                <div class="md:flex">
                    ...
                    <div class="md:w-2/3 p-10 md:p-14 text-left">
                        <div class="flex items-start justify-between gap-6 mb-4">
                            ...
                            <span class=${() => `shrink-0 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                                liveStock() > 0
        ...
        return html`
            <div class="mt-20 relative min-h-screen bg-transparent pt-10 pb-20">
                <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    ...
                    ${() => {
                        if (this.q.status.value === "pending") {
                            return html`<div class="py-40 flex justify-center">${Spinner()}</div>`;
                        }
                        if (this.q.status.value === "error") {
                            return EmptyState({
                                icon: "⚠️",
                                message: "Error al cargar el libro.",
                                action: { label: "Volver al catálogo", onClick: this.handleGoBack },
                            });
                        }
                        return this.renderBook(this.q.data.value);
                    }}
                </div>
            </div>
        `;
